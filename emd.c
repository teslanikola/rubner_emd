		  ...
		  float (*Dist)(feature_t *, feature_t *))
{
  ...
  printf("_EnterX pointer value %p\n", _EnterX);
  printf("_EnterX NextC pointer value %p\n", _EnterX->NextC);
  printf("_EnterX NextR pointer value %p\n", _EnterX->NextR);
  printf("_EndX pointer value %p\n", _EndX);
  printf("-----------------------------------------------------");
  ...
  do
    {
            ...
            for(CurU = uHead.Next; CurU != NULL; CurU = CurU->Next)
      	       printf("[%d,%f]",CurU-Ur,CurU->val);
            printf("\n");
            printf("Vr=");
            for(CurV = vHead.Next; CurV != NULL; CurV = CurV->Next)
      	       printf("[%d,%f]",CurV-Vr,CurV->val);
            printf("\n");
            printf("Delta matrix\n");
            for(int l=0;l<_n1;l++) {
              for(int m=0; m<_n2;m++) {
			     ...
			     node1_t *UHead)
{
  ...
  printf("_EndX's NextC=%p, NextR=%p\n", _EndX->NextC,_EndX->NextR);
  _RowsX[minI] = _EndX;
  _ColsX[minJ] = _EndX;
  printf("_EndX's NextC=%p, NextR=%p\n", _EndX->NextC,_EndX->NextR);
  _EndX++;
